...
def _read_ts_metadata(aretomo_dir, prefix, dose_override=None):
    ...
    # Tilt angles: the .aln TILT column already IS the specimen-referenced
    # tilt when AreTomo3 was run with -TiltCor enabled (confirmed directly,
    # dataset-wide: comparing the same TS processed with -TiltCor 0 vs 1,
    # every TILT value in the TiltCor=1 .aln is shifted from the TiltCor=0
    # (raw nominal, matches the source mdoc exactly) value by precisely
    # that TS's own AlphaOffset -- see CLAUDE.md's alpha_offset convention
    # section). AlphaOffset is only genuinely un-applied when TiltCor was
    # off (in which case it's 0.0 anyway) or after aln-edit (which now also
    # bakes its offset into TILT, matching AreTomo3's own behaviour) -- so
    # the TILT column should always be used directly, never with a further
    # alpha_offset addition on top.
...
def add_parser(subparsers):
    ...
    ext.add_argument('--cut-off', type=float, default=None,
                     help='Override automated LCCmax cut-off (global default; '
                          'overridden per-TS by --cut-off-csv)')
    ext.add_argument('--cut-off-csv', default=None, metavar='CSV',
                     help='CSV file with per-TS LCCmax cut-offs '
                          '(columns: ts_name,threshold). Generated by the '
                          '"Download thresholds CSV" button in the interactive '
                          'QC report. Overrides --cut-off for tomograms listed '
                          'in the file.')
...
def _build_extract_cmd(extract_bin, job_json, args, cut_off_override=None):
    ...
    cut_off = cut_off_override if cut_off_override is not None else args.cut_off
    if cut_off is not None:
        cmd += ['--cut-off', str(cut_off)]
...
def _run_extract_only(args, out_dir, sep):
    ...
    # ── Per-TS threshold CSV ───────────────────────────────────────────────
    per_ts_cutoff = {}
    thr_csv = getattr(args, 'cut_off_csv', None)
    if thr_csv:
        per_ts_cutoff = _load_threshold_csv(thr_csv)
        print(f'Loaded per-TS cut-offs from {thr_csv} ({len(per_ts_cutoff)} entries)')
    ...
    for i, (ts_name, job_json) in enumerate(jobs):
        ...
        cut_off_override = per_ts_cutoff.get(ts_name)
        if cut_off_override is not None:
            print(f'  cut-off (from CSV): {cut_off_override}')
        cmd = _build_extract_cmd(extract_bin, job_json, args,
                                 cut_off_override=cut_off_override)
        _print_cmd(cmd)
